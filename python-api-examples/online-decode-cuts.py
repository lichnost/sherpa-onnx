...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="The number of cuts to decode.",
    )
    ...
    parser.add_argument(
        "--cuts-dir",
        type=str,
        help="The directory of cut files.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="The input cut file(s) to decode.",
    )
...
def main():
    ...
    for cut_file in args.cut_files:
        cuts = CutSet.from_file(Path(args.cuts_dir) / cut_file)
        cuts = cuts.sample(n_cuts=args.n_cuts)
        language = cuts.sample(n_cuts=1).supervisions[0].language
        ...
        total_duration = 0
        for cut in cuts:
            wave_filename = cut.recording.sources[0].source
        ...
        rtfs.append(rtf)
        print(
            f"language: {language} Real time factor (RTF): {elapsed_seconds:.3f}/{total_duration:.3f} = {rtf:.3f}"
        )
    avg_rft = sum(rtfs) / len(rtfs)
    print(f"avg rtf: {avg_rft:.3f}")
